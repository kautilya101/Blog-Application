...
const createPost = async(req:Request, res: Response) => {
  ...
  try{
    ...
    return res.status(201).json({success:true, posts: createdPost})
...
const updatePost = async(req:Request, res: Response) => {
  ...
  try{
    ...
    return res.status(200).json({success:true,posts: updatedPost, message: 'post updated successfully'});
  }catch(e){
    return res.status(500).json({error: 'Internal Server Error'});
  }
}
...
const addComment = async(req:Request, res:Response) => {
  ...
  try{
    ...
    return res.status(500).json({error: 'Error while adding comment'+ e});
  }
...
const viewCount = async(req:Request, res:Response) => {
  ...
  try{
    ...
    return res.status(200).json({success:true, message: 'view increased successfully'});
  }catch(e){
    return res.status(500).json({error: 'Internal Server Error'});
  }
}
